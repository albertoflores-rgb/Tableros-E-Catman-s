...
# cat/acc si vienen por partida doble desde build_merge.py (sufijo '' =
# todos, '_sin_baja' = excluye Status='D') -- build() se llama 2 veces
# para que el toggle 'excluir bajas' del dashboard alterne entre las dos
# vistas sin reimplementar la agregacion en JS (peticion de Alberto,
...
def build(suffix: str) -> dict:
    ...
    cat = pd.read_csv(f'cat_agg{suffix}.csv')
    acc = pd.read_csv(f'accionables_items{suffix}.csv')

    # ---------- KPIs globales (todo Abarrotes, 6 categorias) ----------
    tot_com_mtd = frame['Com_Pesos_MTD'].sum()
    tot_com_mtdly = frame['Com_Pesos_MTDLY'].sum()
    tot_piso_mtd = frame['Piso_Pesos_MTD'].sum()
    tot_piso_mtdly = frame['Piso_Pesos_MTDLY'].sum()
    tot_com_l7d = frame['Com_Pesos_L7D'].sum()
    tot_com_l7dly = frame['Com_Pesos_L7DLY'].sum()
    tot_piso_l7d = frame['Piso_Pesos_L7D'].sum()
    tot_piso_l7dly = frame['Piso_Pesos_L7DLY'].sum()

    # Socios .com -- SOLO ecom (Piso/Brick no tiene grano de socio, ver
    # nota en query_item_total_abarrotes.sql / cte_socios_cat). El total
    # es la SUMA de los valores YA CORRECTOS por categoria en cat_agg
    # (cada uno es un COUNT DISTINCT real hecho en BigQuery) -- es un
    # APROXIMADO honesto: si un mismo socio compra en 2 categorias de
    # Abarrotes se cuenta 2 veces. Se etiqueta como tal en el frontend
    # (badge "Piso: en construccion") para no aparentar mas precision de
    # la que hay. OJO: NO cambia entre 'todos'/'sin_baja' -- limite conocido.
    ...
    kpis = {
        ...
        "n_items": int(frame['Item_Nbr'].nunique()),
        "n_parrilla": int(frame['En_Parrilla'].sum()),
        "n_promo_vigente": int(frame['Promo_Vigente'].sum()),
        "n_accionables": int(frame['Accionable'].sum()),
...
print("KPIs (sin_baja):", json.dumps(data['sin_baja']['kpis'], indent=2, ensure_ascii=False))
print("\nCategorias:", len(data['todos']['categorias']))
