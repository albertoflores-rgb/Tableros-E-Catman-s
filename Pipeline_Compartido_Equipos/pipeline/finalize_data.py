...
# -- cat/riesgo/impulsar/replicar/acc si vienen por partida doble desde
# build_merge.py (sufijo '' = todos, '_sin_baja' = excluye Status='D'),
# asi el dashboard puede alternar entre las dos vistas via un toggle sin
# reimplementar la agregacion en JS (peticion de Alberto, 09-sep-2026).
...
def build(suffix: str) -> dict:
    cat = pd.read_csv(out_dir / f'cat_agg{suffix}.csv')
    riesgo = pd.read_csv(out_dir / f'movers_riesgo{suffix}.csv')
    impulsar = pd.read_csv(out_dir / f'movers_impulsar{suffix}.csv')
    replicar = pd.read_csv(out_dir / f'movers_replicar{suffix}.csv')
    acc = pd.read_csv(out_dir / f'accionables_items{suffix}.csv', low_memory=False)
    ...
    # la que hay. OJO: Socios_Cat_* NO cambia entre 'todos'/'sin_baja'
    # (viene de BQ agregado por categoria, no por item) -- limite conocido.
...
print(f"[{team_key}] KPIs (todos):", json.dumps(data['todos']['kpis'], indent=2, ensure_ascii=False))
print(f"[{team_key}] KPIs (sin_baja):", json.dumps(data['sin_baja']['kpis'], indent=2, ensure_ascii=False))
print(f"[{team_key}] Categorias:", len(data['todos']['categorias']))
