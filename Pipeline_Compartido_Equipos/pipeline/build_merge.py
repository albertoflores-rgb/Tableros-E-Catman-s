...
df = bq.copy()
n_baja = int((df['Status'] == 'D').sum())
print(f"[{team_key}] Items totales: {len(df)} (de los cuales {n_baja} son Status='D' / baja)")
# NO se descartan aqui -- se conservan SIEMPRE en merged_full.csv para
# que el Explorador BQ (que ya trae 'Status' como filtro multi-select)
# los pueda mostrar u ocultar bajo demanda. cat_agg/movers/accionables
# de Resumen y Septiembre SI se calculan por partida doble (ver
# aggregate() abajo) para ofrecer el mismo toggle 'excluir bajas' ahi,
# sin tener que reimplementar la agregacion en JS (peticion de Alberto,
...
def aggregate(frame: pd.DataFrame, suffix: str) -> None:
    """Calcula cat_agg/movers/accionables para un universo de items dado
    y los guarda con el sufijo indicado ('' = todos, '_sin_baja' =
    excluyendo Status='D'). Se llama 2 veces para que el toggle 'excluir
    bajas' del dashboard (Resumen + Septiembre) pueda cambiar de vista
    sin recalcular nada en el navegador -- la logica de agregacion vive
    en un solo lugar (aqui), no duplicada en JS.
    """
    cat_agg = frame.groupby(['Cat_Nbr', 'Cat_Desc']).agg(
        ...
        # items de la categoria (ese era el bug original). OJO: al
        # excluir Status='D', este numero de socios NO cambia (sigue
        # siendo el de TODO el universo real de BQ) -- es un limite
        # conocido de la vista 'sin_baja', no algo a resolver aqui.
    ...
    riesgo_full = (
        frame[(frame['Semaforo_OH'].isin(['OOS', 'Crítico (<6)'])) & (frame['Com_Pesos_MTD'] > 0)]
        .sort_values('Piso_Pesos_MTD', ascending=False)
    )
    impulsar_full = (
        frame[frame['Crecimiento_Com_Pesos_MTD'] <= -0.10]
        .sort_values('Piso_Pesos_MTD', ascending=False)
    )
    replicar_full = (
        frame[frame['Crecimiento_Com_Pesos_MTD'] >= 0.20]
    ...
    print(f"[{team_key}] ({etiqueta}) Movers (top 20 GLOBAL por volumen, SIN gate de parrilla/promo):")
    ...
    cat_agg.to_csv(out_dir / f'cat_agg{suffix}.csv', index=False, encoding='utf-8-sig')
    riesgo.to_csv(out_dir / f'movers_riesgo{suffix}.csv', index=False, encoding='utf-8-sig')
    impulsar.to_csv(out_dir / f'movers_impulsar{suffix}.csv', index=False, encoding='utf-8-sig')
    replicar.to_csv(out_dir / f'movers_replicar{suffix}.csv', index=False, encoding='utf-8-sig')
    accionables_items.to_csv(out_dir / f'accionables_items{suffix}.csv', index=False, encoding='utf-8-sig')
