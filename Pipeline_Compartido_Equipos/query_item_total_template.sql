-- ============================================================
-- query_item_total_template.sql
-- Canal   : Sam's Club MX — Todos los Clubs (176)
-- Área    : E-Catman (PLANTILLA GENERICA -- ver nota abajo)
-- Versión : 1.0 | 01-sep-2026
--
-- QUE ES ESTO -- por que existe un archivo aparte del de Abarrotes:
--   Este archivo es una copia de
--   `tablero_insights_com_abarrotes/query_item_total_abarrotes.sql`
--   (v4.2: 4 momentos YTD/MTD/L7D/AMX, grano Item Total) con UN SOLO
--   cambio: el filtro de categorias del WHERE final ya NO esta
--   hardcoded, es un parametro `@cat_filter` (ARRAY<INT64>).
--
--   Por que: Alberto pidio el mismo ejercicio de tablero para 6 areas
--   mas de E-Catman (Perecederos, Impulso, Seasonal, Apparel,
--   Tecnologia, Salud y Bienestar), cada una con su propia lista de
--   categorias. Clonar este archivo de 500+ lineas 6 veces hubiera
--   significado que cualquier fix futuro a la logica (como el bug de
--   Fecha_Inicio/Fecha_Fin duplicando filas, ya corregido una vez)
--   se tendria que repetir a mano en 6-7 archivos. Con el filtro como
--   parametro, es UN SOLO archivo -- el ajuste de categorias por
--   equipo vive en `teams_config.py`, no aqui.
--
--   El query de Abarrotes (query_item_total_abarrotes.sql) se dejo
--   INTACTO a proposito -- ya esta probado, programado en la rutina
--   diaria de las 8:30 AM, y no hacia falta arriesgar esa automatizacion
--   ya funcionando solo para des-hardcodear un filtro. Si algun dia se
--   quiere unificar Abarrotes tambien bajo este template, es una
--   migracion aparte (cambiar su run_query.py para pasar @cat_filter).
--
--   Como correrlo: NO se ejecuta este .sql suelto con `bq query` sin
--   mas -- necesita el query parameter `@cat_filter`. Usar
--   `run_query_team.py <team_key>` (lee teams_config.py, arma el
--   parametro, ejecuta, guarda el CSV). Ver README.md de esta carpeta.
--
-- Todo lo demas (logica de ventas, inventario, los 4 momentos
-- YTD/MTD/L7D/AMX, comentarios de bloques heredados) es IDENTICO a
-- query_item_total_abarrotes.sql v4.2 -- no se repite el detalle aqui,
-- ver ese archivo para el historial completo de por que cada bloque
-- esta como esta.
-- ============================================================
...
SELECT
  ...
  T2.Item_Nbr, T2.Item_Nbr_Raw, T2.UPC, T2.Item_Desc_1, T2.Item_Desc_2,
